...
#import "MpersonalMsgViewController.h"
...
@interface MpersonalMsgViewController ()

@end

@implementation MpersonalMsgViewController
...
- (void)textViewDidChange:(UITextView *)textView {
    CGFloat fixedWidth = textView.frame.size.width + 20.0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self isMeOrOther:indexPath]) {
        MmeTableViewCell *cell = (MmeTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"meMsgCell" forIndexPath:indexPath];
        if (cell == nil) {
            cell = [[MmeTableViewCell alloc] initWithFrame:CGRectZero];
        }
        [self configureCellme:cell atIndexPath:indexPath];
        return cell;
    } else {
        MOtherTableViewCell *cell = (MOtherTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"otherMsgCell" forIndexPath:indexPath];
        if (cell == nil) {
            cell = [[MOtherTableViewCell alloc] initWithFrame:CGRectZero];
        }
...
- (void)configureCellme:(MmeTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
...
- (void)configureCellother:(MOtherTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
      ...
      newIndexPath:(NSIndexPath *)newIndexPath
{
    ...
    switch(type) {
        ...
        case NSFetchedResultsChangeUpdate:
            if ([[tableView cellForRowAtIndexPath:indexPath] isKindOfClass:[MmeTableViewCell class]]) {
                [self configureCellme:(MmeTableViewCell*)[tableView cellForRowAtIndexPath:indexPath] atIndexPath:indexPath];
            } else if ([[tableView cellForRowAtIndexPath:indexPath] isKindOfClass:[MOtherTableViewCell class]]) {
                [self configureCellother:(MOtherTableViewCell*)[tableView cellForRowAtIndexPath:indexPath] atIndexPath:indexPath];
            }
            break;
